...
export const ChatBot: React.FC = () => {
  ...
  const fileToBase64 = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      // Kết quả sẽ có dạng: "data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,UEsDBB..."
  ...
  const handleSendMessage = async () => {
    ...
    setUploadedFiles([]);
    try {
      // 1. Chuyển đổi các file sang định dạng Flowise yêu cầu (Base64 thuần)
      const flowiseUploads = await Promise.all(
        currentFiles.map(async (f) => {
          let base64Data = "";
          if (f.file) {
            // Luôn ưu tiên dùng File object để convert mới nhất
            base64Data = await fileToBase64(f.file);
          } else if (f.preview && f.preview.startsWith('data:')) {
            // Nếu không có File object nhưng preview đã là data url
            base64Data = f.preview;
          }
          return {
            data: base64Data, // Flowise bắt buộc cần chuỗi này để thực hiện hàm .split(',')
            type: 'file',
            name: f.name,
      ...
      // 2. Tạo tin nhắn hiển thị trên giao diện người dùng
      ...
      // 3. Gọi API với dữ liệu uploads đã chuẩn hóa
      const response = await floWiseService.query(
        userInputContent,
        currentConversationId || 'default-session',
        flowiseUploads
      );

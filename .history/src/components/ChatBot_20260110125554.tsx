...
export const ChatBot: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f1f5f9] overflow-hidden font-sans">
      ...
      <footer className="bg-white/80 backdrop-blur-md p-6 shrink-0 z-20 border-t border-slate-100">
        <div className="max-w-6xl mx-auto relative">

          <div className="absolute bottom-full left-0 w-full px-2 mb-2">
            <FilePreview files={uploadedFiles} onRemove={(id) => setUploadedFiles(f => f.filter(x => x.id !== id))} />
          </div>

          <div className="bg-white border-2 border-slate-100 rounded-[30px] p-3 flex items-end gap-3 shadow-2xl shadow-blue-900/5 focus-within:border-blue-200 focus-within:ring-4 focus-within:ring-blue-500/5 transition-all">
            ...
            <textarea
              ...
              className="flex-1 bg-transparent border-none outline-none text-[16px] py-3 px-2 resize-none max-h-40 text-slate-700 font-medium placeholder:text-slate-400 placeholder:italic"
            />

            <button
              onClick={handleSendMessage}
              disabled={!inputValue.trim() && !uploadedFiles.length}
              className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#1e5b8d] to-[#2a7bb7] text-white flex items-center justify-center shadow-lg shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-20 disabled:shadow-none mb-1 mr-1 group"
            >
          ...
          <div className="flex justify-between items-center mt-4 px-4">
            <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">© Bệnh viện 199 - Đà Nẵng</span>

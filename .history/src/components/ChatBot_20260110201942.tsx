...
export const ChatBot: React.FC = () => {
  ...
  const handleSelectConversation = (id: string) => {
    ...
    if (conversation) {
      ...
      // Nếu danh sách tin nhắn chỉ có 1 (tin chào của bot), tức là chưa gửi câu hỏi nào
  ...
  const autoSaveConversation = (updatedMessages: Message[], userMessage?: string) => {
    if (currentConversation) {
      ...
      if (conversation.title === 'Cuộc trò chuyện mới' && userMessage) {
      ...
      // Cập nhật lại danh sách bên Sidebar
      const savedConversations = storageService.getAllConversations();
      setConversations(savedConversations);
    }
  }
  const handleSendMessage = async () => {
    ...
    const userInputContent = inputValue; // Lưu lại nội dung để đặt tiêu đề
    ...
    // Lưu tin nhắn user và Đổi tiêu đề nếu cần
    ...
    try {
      ...
      autoSaveConversation(finalMessages); // Lưu phản hồi của bot
    } catch (error) {

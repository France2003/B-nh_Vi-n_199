...
export const ChatBot: React.FC = () => {
  ...
  const generateBotResponse = (userMessage: Message): string => {
    if (userMessage.files && userMessage.files.length > 0) {
      ...
      return `✓ Đã nhận ${fileCount} tệp: ${fileNames}. ${userMessage.content
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      {/* Header */}
      <header className="bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 text-white px-4 py-3 sm:px-6 sm:py-4 shadow-lg">
        <div className="flex items-center gap-3">
          <div className="text-3xl">🏥</div>
          <div>
            <h1 className="text-lg sm:text-2xl font-bold">Bệnh viện 199 Đà Nẵng</h1>
          ...
          <div className="flex gap-3 items-center">
            <div className="flex-1 flex items-center gap-3 bg-white border border-gray-200 rounded-2xl px-3 py-2 transition-all focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-100">
              ...
              <input
                ...
                className="flex-1 h-11 sm:h-10 bg-transparent text-gray-900 placeholder-gray-500 focus:outline-none text-sm"
                disabled={isLoading}
              />
            ...
            <button
              ...
              className="w-11 h-11 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold transition-all duration-200 hover:shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"

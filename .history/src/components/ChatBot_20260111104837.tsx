...
export const ChatBot: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const currentFiles = [...uploadedFiles]; // Giữ bản sao file để xử lý
    setIsLoading(true);
    setInputValue('');
    setUploadedFiles([]);
    // 1. Chuyển đổi các file đã upload sang định dạng Flowise yêu cầu
    const flowiseUploads = await Promise.all(
      currentFiles.map(async (f) => ({
        data: f.file ? await fileToBase64(f.file) : "", // Dùng hàm đã fix ở trên
        type: 'file',
        name: f.name,
    ...
    try {
      // Call Flowise API
      const response = await floWiseService.query(userInputContent, currentConversationId || 'default-session', flowiseUploads);

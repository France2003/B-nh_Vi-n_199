...
export const ChatBot: React.FC = () => {
  ...
  const handleNewConversation = () => {
    ...
    setSessionId(`session-${Date.now()}`); // Reset sessionId for new conversation
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Extract File objects from uploadedFiles for sending
      const filesToSend = uploadedFiles.map(uf => uf.file);
      
      // Call Flowise API with question, files, and sessionId
      const response = await floWiseService.query(userInputContent, filesToSend, sessionId);

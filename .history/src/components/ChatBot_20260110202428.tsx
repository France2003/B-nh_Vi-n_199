...
export const ChatBot: React.FC = () => {
  ...
  // FIX: Nhận diện cuộc trò chuyện mới dựa trên tin nhắn thực tế
  const handleSelectConversation = (id: string) => {
    ...
    if (conversation) {
      ...
      // Kiểm tra xem trong lịch sử đã có tin nhắn nào của User chưa
  ...
  // FIX: Cập nhật tiêu đề và đồng bộ danh sách Sidebar ngay lập tức
  const autoSaveConversation = (updatedMessages: Message[], userMessage?: string) => {
    if (currentConversation) {
      let conversation = { ...currentConversation }; // Copy để tránh mutation trực tiếp

      // Update title nếu là lần đầu người dùng gửi tin nhắn
      ...
      // Đồng bộ lại danh sách conversations để tiêu đề mới hiện lên Sidebar

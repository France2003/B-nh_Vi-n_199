...
app.post('/api/flowise', async (req, res) => {
  ...
  try {
    ...
    console.log(`[HỆ THỐNG: ĐÃ ĐÍNH KÈM ${uploads.length} FILE] ${question}`);
    console.log('Session ID:', sessionId);
    console.log('Files received:', uploads ? uploads.length : 0);
    ...
    try {
      ...
      const response = await fetch(floWiseUrl, {
        ...
        body: JSON.stringify({
          question: `[HỆ THỐNG: ĐÃ ĐÍNH KÈM ${uploads.length} FILE] ${question}`,
          chatId: sessionId,
          uploads: uploads || [],
          overrideConfig: {
            vars:{

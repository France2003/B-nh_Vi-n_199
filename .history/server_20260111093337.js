...
app.post('/api/flowise', upload.any(), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Agent node cần format message đặc biệt - gửi dưới dạng message history
      // Format: [{ role: "user", content: "question" }]
      const message = JSON.stringify([{ role: 'user', content: question }]);
      floWiseFormData.append('chatHistory', message);

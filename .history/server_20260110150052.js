...
const API_TIMEOUT = 5 * 60 * 1000; // 5 minutes for long-running flows
...
app.get('/test-flowise', async (req, res) => {
  ...
  try {
    const testResponse = await fetch(FLOWISE_API_URL, {
      method: 'POST',
      headers: {
      ...
      signal: AbortSignal.timeout(10000), // 10 second test timeout
    });
    ...
    console.error('❌ Flowise test failed:', error.message);
    res.status(503).json({
      status: 'error',
...
app.post('/api/flowise', async (req, res) => {
  ...
  try {
    ...
    const timeoutId = setTimeout(() => {
      console.log('⏰ Timeout triggered after', API_TIMEOUT / 1000, 'seconds');
      controller.abort();
    }, API_TIMEOUT);
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        console.error('⏱️ Flowise API Timeout (>5min)');
        return res.status(504).json({
          error: 'Flowise API Timeout',
          text: `Flow xử lý quá lâu (>${API_TIMEOUT / 60000} phút). Vui lòng kiểm tra flow trong Flowise.`,
          elapsed: `${elapsed}s`,
        });
      }

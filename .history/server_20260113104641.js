...
const FLOWISE_API_URL = 'https://flowise.imagentu.cloud/api/v1/prediction/3acfb108-785f-4149-846a-0853f164c562';
...
app.post('/api/flowise', async (req, res) => {
  ...
  try {
    const { question, sessionId, uploads } = req.body;

    if (!question && (!uploads || uploads.length === 0)) {
      return res.status(400).json({
        error: 'Question is required',
        text: 'Vui lòng nhập nội dung câu hỏi',
        uploads: 'Nội dung hoặc file là bắt buộc'
      });
    }
    const fileNames = uploads.map(file => file.name || 'File-khong-ten').join(', ');
    ...
    try {
      ...
      // const systemContext = fileCount.length > 0 ? `[HỆ THỐNG: ĐÃ ĐÍNH KÈM ${fileCount} FILE: ${fileNames}] ` : '';
      const systemContext = fileCount.length > 0 ? `[HỆ THỐNG: Có ${fileCount.length} file đính kèm trong yêu cầu này: ${fileNames}]. 
     Bạn hãy SỬ DỤNG CÔNG CỤ ĐỌC FILE để phân tích nội dung bên trong các file này trước khi trả lời người dùng.`
  : question;
      // Add longer timeout to fetch itself
      const response = await fetch(floWiseUrl, {
        ...
        body: JSON.stringify({
          question: `${systemContext}${question}`,
          chatId: sessionId,
          uploads: uploads || [],
          overrideConfig: {
            vars:{
      ...
      res.json({data,processedFiles: fileNames});
    } catch (fetchError) {
      clearTimeout(timeoutId);
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);

    ...
    try {
      ...
      const flowiseUploads = await Promise.all(
        currentFiles.map(async (f) => {
          ...
          return {
            data: base64Data || "data:application/octet-stream;base64,", // Đảm bảo luôn có dấu phẩy để không lỗi split
            type: 'file',
            name: f.name,
      ...
      const response = await floWiseService.query(
        userInputContent,
        currentConversationId || 'session-' + Date.now(), // Đảm bảo sessionId ổn định
        flowiseUploads
      );
      ...
      const botMessage: Message = {
        ...
        content: botContent,
        timestamp: new Date(),
        notebookLink: response.notebookLink || response.link_notebook || null,
      };

      const updatedMessages = [...newMessages, botMessage];
      setMessages(updatedMessages);
      autoSaveConversation(updatedMessages);
      console.log("Dữ liệu gửi lên Flowise:", { userInputContent, flowiseUploads });
    } catch (error) {
